...
export default function ChatPage({ chat, messages }) {
  ...
  const handleSendMessage = (e) => {
    ...
    if (input.trim().length >= 1) {
      ...
      // send message
      db.collection('clubs').doc(id).collection('messages').add({
...
export async function getServerSideProps(context) {
  const ref = db.collection('clubs').doc(context.query.id)
  ...
  const chat = {
    id: chatRes.id,
    admin: chatRes.data().admin,
    name: chatRes.data().name,
    dp: chatRes.data().dp,
  }
